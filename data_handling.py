...
def main():
    ...
    if parsed_input[1][1].lower() == "f":
        if len(parsed_input[1]) < 4:
            errors.append(terminal_output("error", "Invalid input format. Second line must contain at least 4 values when the specified data-type is \"f\"."))
        if not isinstance(parsed_input[1][2], int):
            errors.append(terminal_output("error", "Invalid input format. Third value of the second line must be an integer when the specified data-type is \"f\"."))
        if not isinstance(parsed_input[1][3], int):
            errors.append(terminal_output("error", "Invalid input format. Fourth value of the second line must be an integer when the specified data-type is \"f\"."))
        
    if parsed_input[1][1].lower() == "c":
        if len(parsed_input[1]) < 3:
            errors.append(terminal_output("error", "Invalid input format. Second line must contain at least 3 values when the specified data-type is \"c\"."))
        if not isinstance(parsed_input[1][2], int):
            errors.append(terminal_output("error", "Invalid input format. Third value of the second line must be an integer representing test length or maximum possible test score when the specified data-type is \"c\"."))
        if len(parsed_input[1]) == 3:
            parsed_input[1].append(0)
            notes.append(terminal_output("note", "Fourth value of the second line not specified. Defaulting to 0."))
        if not isinstance(parsed_input[1][3], (float, int)):
            errors.append(terminal_output("error", "Invalid input format. Fourth value of the second line must be a number representing minimum possible test score when the specified data-type is \"c\"."))   
...
def prep_data(data_path: str, input_type: str) -> list:
    ...
    if not os.path.exists(data_path):
        return terminal_output("error", f"Data file \"{file_name}\" not found at \"{base_path}\".") 

    if input_type.lower() not in ["r", "f", "m", "c"]:
        return terminal_output("error", f"Invalid input type ({input_type}). Valid input types are \"r\" for raw data, \"f\" for frequency data, \"m\" for moment data, and \"c\" for complete data.")
